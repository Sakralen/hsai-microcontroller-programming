...
/*static*/  CPU_FNCT_VOID  BSP_IntVectTbl[BSP_INT_SRC_NBR];
  //  После объявления эта таблица заполнена "нулевыми" указателями, но
  // функция BSP_IntInit() (строка 281 и далее) заполняет элементы таблицы адресом пустого
  // "обработчика-заглушки" BSP_IntHandlerDummy, который определен в данном файле, строки 417-420
...
{
    CPU_IntDis();         //  Определение в файле cpu_a.asm (строка 76) (APN)
}
...
*
* Note(s)     : Если нигде ничего не менять, то при возникновении любого запроса будет вызываться
  пустая функция void  BSP_IntHandlerDummy (void) - строка 417 в данном файле.
*********************************************************************************************************
*/
...
static  void  BSP_IntHandler (CPU_DATA  int_id)
{
    ...
    CPU_FNCT_VOID  isr;
        ...
        //  Нужно обязательно учитывать глубину вложенности (ее используетс Ядро ОСРВ)
    CPU_CRITICAL_ENTER();               /* Tell uC/OS-II that we are starting an ISR            */
    OSIntNesting++;                     //  Инкремент счетчика вложенности прерываний
    CPU_CRITICAL_EXIT();                //  Конец критической секции
                        //  Нужно обязательно проверять корректность параметров, иначе все может рухнуть
    if (int_id < BSP_INT_SRC_NBR) {     //    int_id не должен превышать BSP_INT_SRC_NBR, если да, то
        isr = BSP_IntVectTbl[int_id];   //  выборка указателя на функцию пользователя из таблицы (псевдовекторов)
        if (isr != (CPU_FNCT_VOID)0) {  //    если прочитанный указатель не NULL, если ДА, то
            isr();                      //  вызов пользовательского обработчика через указатель
        }
    }
                  //  Выход из обработчика нужно обязательно делать через функцию OSIntExit()
