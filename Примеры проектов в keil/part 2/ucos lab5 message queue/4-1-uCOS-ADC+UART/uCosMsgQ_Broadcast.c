...
//	args1ch.discrPeriod = 123;
	...
	if (0 == (pQueue = OSQCreate(&queueArr[0], QUEUE_SIZE))) {
		__NOP();
	}
...
static void App_TaskMeas (void *p_arg) {
	...
	while (DEF_TRUE) {
		...
		if(args->seriesLength == (++curSeriesCnt)) {
			...
			partialSum = 0;
#ifdef USE_BROADCAST
			if (OS_MBOX_FULL == OSQPostOpt(pQueue, &resultOut, OS_POST_OPT_BROADCAST)) {
#else	
			if (OS_MBOX_FULL == OSQPostOpt(pQueue, &resultOut, OS_POST_OPT_NONE)) {
#endif				
        __NOP();
      }
			OSQQuery(pQueue, &qData);
      queueElemCnt = qData.OSNMsgs;
      if (QUEUE_SIZE == queueElemCnt) {
        __NOP();
      }
		}			
...
static void App_TaskSend1 (void *p_arg) {
	...
	while (DEF_TRUE) {
		resultIn = *((u32*)OSQPend(pQueue, 0, &err));		
		sendOsc += SEND_OSC;
		Write1_Poll((s8*)"Channel ");
		Dec_Convert((s8*)(buffer), (s8)((resultIn >> 16) + 1));
