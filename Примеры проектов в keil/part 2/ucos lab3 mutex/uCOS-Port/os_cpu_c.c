...
void  OS_CPU_SysTickInit (void)
{
    ...
    cnts = OS_CPU_SysTickClkFreq() / OS_TICKS_PER_SEC;  //  Опр. константы перезагрузки для SysTick

    OS_CPU_CM3_NVIC_ST_RELOAD = (cnts - 1); // и запись ее в рег.перезаг.  (>>1 - АПН (2015.02.18))
                                                 /* Enable timer.                                      */
    OS_CPU_CM3_NVIC_ST_CTRL  |= OS_CPU_CM3_NVIC_ST_CTRL_CLK_SRC | OS_CPU_CM3_NVIC_ST_CTRL_ENABLE;
                                                 /* Enable timer interrupt.  -- пуск SysTick таймера                           */
